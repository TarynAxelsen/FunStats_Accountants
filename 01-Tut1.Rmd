...
knitr::include_graphics("Q1_Excel.jpg")
```
...
```{r make-both-conditional-graphs, echo=FALSE, message=FALSE, warning=FALSE, results='hide'}
# Ensure figures folder exists in _book
dir.create("_book/figures", showWarnings = FALSE)

# ---------- Graph 1 ----------
png("_book/figures/conditional_satisfaction_given_client.png",
    width = 800, height = 500)

cond_satisfaction_client <- matrix(
  c(61.29, 38.71,
    32.76, 67.24),
  nrow = 2, byrow = TRUE
)

rownames(cond_satisfaction_client) <- c("New Client", "Old Client")
colnames(cond_satisfaction_client) <- c("Satisfied", "Dissatisfied")

cols <- c("#a6cee3", "#1f78b4")

par(mar = c(7, 4, 4, 2) + 0.1)
bp <- barplot(
  t(cond_satisfaction_client),
  col = cols,
  ylim = c(0, 100),
  ylab = "Percentage",
  xlab = "Type of Client",
  main = "Satisfaction Level given Type of Client",
  las = 1
)

for (i in seq_len(ncol(cond_satisfaction_client))) {
  cum <- 0
  for (j in seq_len(nrow(cond_satisfaction_client))) {
    v <- cond_satisfaction_client[i, j]
    text(bp[i], cum + v/2, paste0(v, "%"),
...
legend("bottom", inset=-0.25,
       legend=c("Satisfied", "Dissatisfied"),
       fill=cols, horiz=TRUE, bty="n", xpd=TRUE)

invisible(dev.off())


# ---------- Graph 2 ----------
png("_book/figures/conditional_client_given_satisfaction.png",
    width = 800, height = 500)

cond_client_satisfaction <- matrix(
  c(75, 25,
    48, 52),
  nrow = 2, byrow = TRUE
)

rownames(cond_client_satisfaction) <- c("Satisfied", "Dissatisfied")
colnames(cond_client_satisfaction) <- c("New Client", "Old Client")

cols2 <- c("#b2df8a", "#33a02c")

par(mar = c(7, 4, 4, 2) + 0.1)
bp2 <- barplot(
  t(cond_client_satisfaction),
  col = cols2,
  ylim = c(0, 100),
  ylab = "Percentage",
  xlab = "Level of Satisfaction Level",
  main = "Level of Client Type given Satisfaction Level",
  las = 1
)

for (i in seq_len(ncol(cond_client_satisfaction))) {
  cum <- 0
  for (j in seq_len(nrow(cond_client_satisfaction))) {
    v <- cond_client_satisfaction[i, j]
    text(bp2[i], cum + v/2, paste0(v, "%"),
         col="black", cex=1.2, font=2)
    cum <- cum + v
  }
}
...
legend("bottom", inset=-0.25,
       legend=c("New Client", "Old Client"),
       fill=cols2, horiz=TRUE, bty="n", xpd=TRUE)
